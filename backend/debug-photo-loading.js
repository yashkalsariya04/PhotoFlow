...
async function debugPhotoLoading() {
  ...
  try {
    ...
    try {
      ...
      const clientResponse = await fetch(`https://facematrix.sonomainfotech.in/api/events/client/${access._id}`);
      ...
      if (clientResponse.ok) {
        ...
        if (result.photos && result.photos.length > 0) {
          ...
          for (let i = 0; i < Math.min(result.photos.length, 3); i++) {
            ...
            const thumbnailUrl = `https://facematrix.sonomainfotech.in/api/photos/${photo._id}/thumbnail/public`;
