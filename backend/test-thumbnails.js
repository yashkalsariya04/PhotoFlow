...
async function testThumbnails() {
  ...
  try {
    ...
    try {
      const authResponse = await fetch(`https://facematrix.sonomainfotech.in/api/photos/${photoId}/thumbnail`);
    ...
    try {
      const publicResponse = await fetch(`https://facematrix.sonomainfotech.in/api/photos/${photoId}/thumbnail/public`);
