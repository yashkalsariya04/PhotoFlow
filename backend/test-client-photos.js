...
async function testClientPhotos() {
  ...
  try {
    ...
    // Test the client photos endpoint
    const response = await fetch(`https://PhotoFlow.in/api/events/client-photos/${accessId}`);
    ...
    if (response.ok) {
      ...
      if (result.photos.length > 0) {
        result.photos.forEach((photo, i) => {
          ...
          console.log(`  🔗 URL: https://PhotoFlow.in/api/photos/${photo._id}`);
          console.log(`  🖼️ Thumbnail: https://PhotoFlow.in/api/photos/${photo._id}/thumbnail`);
        });
      }
    } else {
